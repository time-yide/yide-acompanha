...
Em `src/lib/freela-yide/schema.ts`, atualizar `criarOportunidadeSchema` e adicionar o helper + tipos. Substituir o bloco do schema por:
...
E no objeto do `.insert({...})`, adicionar (depois de `tipo: parsed.data.tipo,`):

```typescript
    entrega_urgente: urg.entrega_urgente,
    prazo_entrega: urg.prazo_entrega,
```
