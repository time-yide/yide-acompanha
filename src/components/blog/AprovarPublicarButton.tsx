...
import { publicarPostAction } from "@/lib/blog/actions";

/** Aprovação rápida na lista: 1 clique publica (ou despublica) sem abrir o editor. */
export function AprovarPublicarButton({ id, publicado }: { id: string; publicado: boolean }) {
  ...
  function acao() {
    ...
    start(async () => {
      const r = await publicarPostAction(fd);
      if ("error" in r) { toast.error(r.error); return; }
      toast.success(publicado ? "Despublicado" : "Publicado no blog!");
      router.refresh();
    });
  }
  return (
