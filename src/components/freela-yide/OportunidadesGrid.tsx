...
export function OportunidadesGrid({ ops, gestao = false, podePegar = true }: { ops: OportunidadeRow[]; gestao?: boolean; podePegar?: boolean }) {
  if (ops.length === 0) {
    return <Card className="p-8 text-center text-sm text-muted-foreground">Nenhuma oportunidade disponível agora.</Card>;
  }
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {ops.map((op) => <OportunidadeCard key={op.id} op={op} gestao={gestao} podePegar={podePegar} />)}
    </div>
  );
}
