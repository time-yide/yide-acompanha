...
import { publicarCaseAction } from "@/lib/seo/case-actions";

/** Aprovação rápida na lista: 1 clique publica (ou despublica) o case. */
export function PublicarCaseButton({ id, publicado }: { id: string; publicado: boolean }) {
  ...
  function acao() {
    ...
    start(async () => {
      const r = await publicarCaseAction(fd);
