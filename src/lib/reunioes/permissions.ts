...
/**
 * Excluir (soft-delete) uma reunião: quem gravou OU gestão. Mesma audiência
 * de quem já enxerga a reunião — assessor não apaga reunião de outro assessor.
 */
export function podeExcluirReuniao(
  user: { id: string; role: string },
  meeting: { owner_user_id: string },
): boolean {
  return podeVerReuniao(user, meeting);
}
