...
import { ROLES_VOZ_IA, ROLES_CONFIG_VOZ_IA } from "./types";
...
export async function testarVozIAAction(telefone: string): Promise<LigarResult> {
  ...
  if (!telefone?.trim()) return { error: "Informe o número" };
  ...
  if (!config) return { error: "Configure o agente de voz primeiro" };
  ...
  const { data: instancia } = await sb
    ...
    .limit(1)
    .maybeSingle();
  if (!instancia?.numero) return { error: "Nenhuma instância Twilio configurada" };
  ...
  const { data: call, error: insertErr } = await sb
    ...
    .insert({
      ...
      resultado_detalhe: "Ligação de teste",
    })
    .select("id")
    .single();
  ...
  const body = new URLSearchParams({
    To: numero.startsWith("+") ? numero : `+55${numero}`,
